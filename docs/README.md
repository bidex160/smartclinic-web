...
The public shell is role-aware without changing authorization: guests see booking, provider acquisition, and sign-in actions; authenticated users see only entitled patient, provider, or operations portal destinations. Provider and operations dashboards use dedicated backend summary APIs for authoritative operational totals, while compact offer/matching tables remain previews only. Pending providers retain setup access while operational work remains gated by backend status and existing guards. Provider financial reporting remains deferred until an authoritative earnings/settlement domain exists.
...
Standard users can now register, sign in, and use a dedicated patient portal for their safe SELF Patient profile, SmartClinic Patient ID, category-grouped Health Check history, booking details, completed results, and new authenticated SELF bookings. Registered bookings support JWT-authorized Paystack Popup, hosted payment link, and pay-later collection with backend-authoritative verification before automatic matching. Registration never fabricates a session, and the public Patient ID is not a password or record-access credential. Wallet and organisation funding remain deferred, as do guest longitudinal history, guest Patient claiming, family/dependent access, provider Patient-ID consent access, password reset, and profile editing.

Referral progression is direct-only and database-driven. Members can copy patient, clinic, laboratory, and pharmacy invite links, review backend-owned current/next-level requirements and qualified counts, and see ledger-backed points and safe referral history. USER accounts can request manual cash withdrawal; points are reserved while operations transfers externally and records the outcome. Authenticated SELF Health Checks can also reserve points for points-only or split Paystack payment, using server-returned monetary values throughout. SmartClinic does not automatically transfer funds or verify bank ownership. Downlines, referral trees, network earnings, point transfer, and point purchase remain deferred.

The authenticated My Impact page provides one backend-authoritative summary of referral code and invites, verified connections, progression, reward balances, pipeline totals, and optional public-leaderboard position. Participation is explicit and reversible; no legacy Join authentication, level names, milestone cash, missions, downlines, or frontend-derived ranking rules are used.
...
Patients can use API-driven Find Care discovery, submit authenticated Care Requests, track them in My Care, and request FastTrack for supported SmartClinic or externally booked appointments. Provider eligibility, verification, fees, payment, and confirmation remain backend-authoritative.

Accepted General Care requests are scheduled explicitly by providers with start and end times. Providers manage Care Appointments in their existing portal, while patients see the resulting appointment in My Care and use backend appointment-aware cancellation. This workflow remains separate from Health Check scheduling and FastTrack payment.
