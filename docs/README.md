...
Provider onboarding now supports operations create-and-invite and public self-registration. Both create pending provider identities and accounts without implying eligibility. Providers can review/update their permitted profile and submit or resubmit it; operations explicitly approves or rejects submitted onboarding. Approval is distinct from account creation and is the backend-owned transition to operational ACTIVE. Invitation fallback links and passwords remain memory-only, and capability/location/availability plus HOME_VISIT service areas remain deferred.

Operations can configure provider-location matching prerequisites from provider detail: package/mode capabilities, physical locations, capability-location links, weekly availability, and dated availability exceptions. The UI's readiness summary is informational; the matching backend remains authoritative for the actual booking window, exceptions, capacity, and eligibility. HOME_VISIT coverage and provider self-service configuration remain deferred.
