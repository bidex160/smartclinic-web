...
Operations reviews provider-submitted configuration, approves or rejects onboarding, controls suspension/reactivation, and retains configuration tools for exceptional support. Approval and operational ACTIVE status remain backend-owned and distinct from account creation or readiness. The matching backend remains authoritative for actual booking windows, exceptions, capacity, and eligibility. HOME_VISIT service-area coverage remains deferred.
