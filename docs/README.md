...
Referral progression is direct-only and database-driven. Members can copy patient, clinic, laboratory, and pharmacy invite links, review backend-owned current/next-level requirements and qualified counts, and see ledger-backed points and safe referral history. USER accounts can request manual cash withdrawal; points are reserved while operations transfers externally and records the outcome. SmartClinic does not automatically transfer funds or verify bank ownership. Health Check points spending, downlines, referral trees, and network earnings remain deferred.
