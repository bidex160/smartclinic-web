...
| `POST /api/v1/admin/providers/:id/invitations`                   | Create one manually delivered invitation           | Raw token appears only in this creation response                       |
...
Invitation tokens are URL route parameters by design, but are never copied into global state, logs, local storage, or session storage. Admin creation keeps the returned token/link only in component memory for manual sharing; subsequent list calls cannot retrieve it. Public inspection and acceptance opt out of staff bearer authentication. Successful acceptance clears password controls and directs the provider to normal login. Existing-email conflicts return to operations for explicit existing-account linking rather than password overwrite.
