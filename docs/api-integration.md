...
- The legacy admin Package Pricing screen is no longer routed or navigable. Patient-facing Health Check prices are ProviderService capability prices, not global package catalogue prices.
...
Provider encounter requests use the existing bearer session and explicit PROVIDER route guard. The encounter projection supplies snapshot-driven `requirements` with code, display name, category, result type, nullable unit, source, and whether a recorded result is required. Angular renders these requirements by result type and does not reconstruct package composition or completion rules. The measurement request preserves the dedicated systolic/diastolic blood pressure, blood glucose, BMI, temperature, oxygen saturation, and pulse objects, and sends other result-bearing requirements through `additionalResults` as `{ code, value, secondaryValue? }`. It never submits units, `NONE` work items, interpretation, contact, funding, payment, history, or authentication data. Save and complete are distinct non-replayed operations; completion eligibility and the returned read-only state remain backend-authoritative.
...
The provider self-service workspace consumes the parallel `/provider/services`, `/provider/locations`, `/provider/availability`, and `/provider/availability-exceptions` resources. Each Health Check ProviderService is scoped to one package and fulfilment mode and carries its own `priceMinor`/currency. Creation sends that exact capability price, and the provider-owned `/provider/services/:id/price` command updates it with integer-safe minor units; zero means free. The authenticated provider context supplies ownership: provider IDs are absent from routes, query parameters, and request bodies. ProviderServiceLocation links use the provider-owned POST/DELETE routes. Mutations opt out of automatic replay.
...
Care Chat remains anchored to the Care Request public reference. Patient calls use `/me/care-requests/:reference/chat`; provider calls use the parallel `/provider` routes. Message reads are newest-first and are reversed only for chronological presentation. Opening an eligible chat issues the explicit `/read` command, while routed pages poll the newest bounded page about every five seconds with non-overlapping requests. Message bodies stay plain text in component memory; there is no global polling, persistence, markup rendering, attachment support, or sender identity in mutation bodies.

General Care offering configuration is deliberately separate from Health Check `/provider/services`. It uses `/provider/care-services/catalogue` and `/provider/care-services`, with PATCH updates and dedicated activate/deactivate commands. The central definition UUID is used only as the backend-required create selector and is not displayed. Every selected delivery mode has an independent currency and price in `deliveryOptions`; prices are entered as major-unit decimal strings, converted with string-based minor-unit parsing, and sent as safe integers. Zero is an explicit free price, while a blank selected-mode price is invalid. General Care no longer supports price-on-request. FastTrack pricing remains a separate offering-level concern.

The platform General Care catalogue is managed by ADMIN/OPERATIONS through `GET/POST /admin/care-service-definitions` and `PATCH /admin/care-service-definitions/:id`. Creation sends the backend-required `code`, `name`, and optional nullable `description`; the backend creates the definition active. Editing supports those catalogue fields, while activation and deactivation use the same PATCH contract with `isActive`. Provider pricing, delivery modes, appointment support, and FastTrack configuration remain exclusively on `ProviderCareService`.

ADMIN/OPERATIONS commission configuration uses `GET/PATCH /admin/commercial-settings/provider-commission` for the platform default and `GET/PATCH/DELETE /admin/providers/:id/commission` for provider overrides. Angular presents percentages but sends integer `commissionBasisPoints`; conversion parses decimal strings exactly, with `0%` remaining distinct from an unconfigured `null`. Provider effective rate and source are displayed from the backend response. Removing an override restores inheritance and never clears the platform default. Commission is deducted from provider revenue and is not presented as a patient surcharge.

General Care service funding starts only after provider acceptance. Patient reads and commands use `GET /me/care-requests/:reference/funding`, `POST .../funding/initialize`, and `POST .../funding/verify-latest`. The UI displays the immutable Care Request price snapshot and never sends an amount or currency when initializing Paystack. `PENDING`, `PAID`, and `SATISFIED_FREE` are kept separate from the Care Request lifecycle; free care never opens Paystack, while paid or free accepted care can proceed to provider scheduling. Chat remains available after acceptance regardless of funding.

General Care uses the existing authenticated patient return base, so local backend configuration remains `PAYSTACK_PATIENT_CALLBACK_URL=http://localhost:4200/me/payment-return/` with the trailing slash. The route dispatches a trusted `SC-CARE-...` path reference to General Care `verify-latest`; Paystack query parameters are ignored and never treated as payment evidence. FastTrack funding is separate and no combined checkout is performed.
