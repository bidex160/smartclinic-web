...
General Care offering configuration is deliberately separate from Health Check `/provider/services`. It uses `/provider/care-services/catalogue` and `/provider/care-services`, with PATCH updates and dedicated activate/deactivate commands. The central definition UUID is used only as the backend-required create selector and is not displayed. Price and FastTrack amounts are entered as major-unit decimal strings, converted with string-based minor-unit parsing, and sent as safe integers; price-on-request sends the authoritative `null` price/currency pair.
