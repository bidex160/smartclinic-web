...
ADMIN/OPERATIONS provider detail now owns the first provider-location eligibility configuration surface. Its focused sections manage catalogue-backed ProviderService capabilities, named ProviderLocations, explicit ProviderServiceLocation links, recurring weekly availability, and dated availability exceptions. All mutations use the backend's provider-scoped endpoints and opt out of automatic replay. The readiness summary reports only missing minimum configuration; it never predicts eligibility for a booking because matching also evaluates catalogue activity, schedule coverage, exceptions, reservations/capacity, and lifecycle state server-side. Provider self-service configuration and HOME_VISIT service areas remain deferred.
