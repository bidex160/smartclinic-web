...
General Care `ProviderCareService` configuration has its own provider route, models, API adapter, and forms. It does not reuse or mutate the Health Check capability editor. Delivery modes, appointment-request support, standard pricing, FastTrack configuration, and visibility are independent offering dimensions; meeting links remain appointment data and chat remains Care Request data.
