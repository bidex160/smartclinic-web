...
Confirmed provider assignments link to a provider-only Smart Health Check encounter. The page loads or explicitly starts the server-owned encounter and renders the returned snapshot-driven clinical requirements as no-result work items, single numeric inputs, or paired blood-pressure inputs. Saving preserves the six dedicated compatibility fields and sends other result-bearing requirement codes through the backend-defined additional-results shape without duplicates. Units, provenance, persisted values, completion eligibility, and the returned encounter state come from the API; completed encounters disable the form. The frontend provides structural numeric validation only and contains no clinical ranges, interpretations, diagnoses, alerts, or patient-facing results.
...
Care Chat is a routed Care Request sub-feature shared by patient and provider portals. A scope-aware API adapter changes only the authorized route prefix; components never select participants or sender identity. Polling exists only for the active chat component and is destroyed with the route. Chat availability, write access, unread totals, safe participant identity, appointment context, and lifecycle changes remain backend-authoritative.

General Care `ProviderCareService` configuration has its own provider route, models, API adapter, and forms. It does not reuse or mutate the Health Check capability editor. Each `deliveryOptions` entry couples one supported delivery mode to its authoritative currency and price; FastTrack configuration and visibility remain independent offering dimensions. Care Requests display their immutable backend service-price snapshot rather than re-reading the Provider's current offering. Meeting links remain appointment data and chat remains Care Request data.

General Care funding is a Care Request entitlement, not an appointment payment. After provider acceptance, the patient initializes and verifies payment against the Care Request public reference; Angular never supplies the amount or marks payment settled from a popup callback. `PAID` remains satisfied across cancelled/no-show replacement appointments, and `SATISFIED_FREE` bypasses Paystack. Provider scheduling is gated for usability from the backend funding summary, while backend commands remain the final transition authority. FastTrack funding and Health Check funding remain distinct domains.

Country/state/city forms keep persisted geography separate from selector mechanics. `countryCode` remains the ISO country code, while `stateOrRegion` (or the provider-location contract's `state`) and `city` remain human-readable names. State selects use a UI-only reactive FormControl containing the `LocationDataService` state ISO code. Existing records are initialized by matching the persisted state name, setting that UI control without emitting, loading cities by ISO code, and preserving the persisted city; destructive dependent-field resets run only for user country/state changes. UI state codes are never included in API payloads.
