...
@Component({
  ...
  template: `<main class="mx-auto max-w-4xl px-5 py-10 sm:px-8">
    ...
    @if (targeted() && loading()) {
      ...
      <section role="alert" class="mt-8 rounded-2xl bg-red-50 p-6">
        <h2 class="font-bold">This hospital is unavailable</h2>
        <p class="mt-2">
          We couldn't prepare this hospital connection. Please go back and try again.
        </p>
        <a [routerLink]="backUrl()" class="mt-4 inline-flex rounded-xl border px-4 py-2 font-bold"
          >Go back</a
        >
      </section>
      ...
      <section class="mt-7 rounded-[2rem] border border-violet-100 bg-gradient-to-b from-white to-violet-50/40 p-6 shadow-sm">
        ...
        @if (loading()) {
          <p role="status" class="mt-4">Loading hospitals…</p>
        } @else if (directoryError()) {
          <p role="alert" class="mt-4 rounded-xl bg-red-50 p-4">
            Unable to load hospitals.
            <button type="button" (click)="load(page())" class="font-bold underline">
              Try again
            </button>
          </p>
          ...
          <div class="mt-4 flex justify-between">
            <button type="button" (click)="load(page() - 1)" [disabled]="page() <= 1">
              Previous</button
            ><span>Page {{ page() }} of {{ pages() || 1 }}</span
            ><button type="button" (click)="load(page() + 1)" [disabled]="page() >= pages()">
              Next
            </button>
          </div>
        }
      ...
      >
        ...
        <label class="flex gap-3"
          ><input type="checkbox" formControlName="consentAcknowledged" /><span
            >I authorize SmartClinic to send the information needed to register or connect me with
            this hospital.</span
          ></label
        >
        @if (error()) {
...
export class ConnectProviderPageComponent {
  ...
  readonly providerReference = this.route.snapshot.queryParamMap.get('providerReference');
  readonly targeted = signal(this.providerReference !== null);
  readonly returnUrl = this.safeReturnUrl(this.route.snapshot.queryParamMap.get('returnUrl'));
  readonly form = new FormGroup({
    path: new FormControl<PatientProviderConnectionType | null>(null, Validators.required),
    externalPatientReference: new FormControl('', { nonNullable: true }),
    consentAcknowledged: new FormControl(false, {
      nonNullable: true,
      validators: [Validators.requiredTrue],
    }),
  });
  constructor() {
    if (this.providerReference) this.loadTarget(this.providerReference);
    else this.load(1);
  }
  load(page: number) {
  ...
  private loadTarget(reference: string) {
    if (!/^SCPR-[A-F0-9]{32}$/.test(reference)) {
      this.targetError.set(true);
      return;
    }
    this.loading.set(true);
    this.api.listMine(1, 100).subscribe({
      next: (connections) => {
        const existing = connections.items.find(
          (item) => item.provider.providerReference === reference,
        );
        if (existing && !['REJECTED', 'CANCELLED'].includes(existing.status)) {
          if (existing.status === 'CONNECTED' && this.returnUrl) {
            void this.router.navigateByUrl(this.returnUrl);
            return;
          }
          void this.router.navigate(['/me/providers', existing.reference], {
            queryParams: this.returnUrl ? { returnUrl: this.returnUrl } : undefined,
          });
          return;
        }
        this.resolveTarget(reference, 1);
      },
      error: () => {
  ...
  private resolveTarget(reference: string, page: number) {
    this.api.directory('', page, 100).subscribe({
      next: (r) => {
        const exact = r.items.find((item) => item.providerReference === reference);
        if (exact) {
          this.choose(exact);
          this.loading.set(false);
        } else if (page < r.totalPages) this.resolveTarget(reference, page + 1);
        else {
          this.loading.set(false);
          this.targetError.set(true);
        }
  ...
  choose(p: PatientProviderConnectionDirectoryItem) {
    this.selected.set(p);
    const path = p.newPatientRegistration.enabled
  ...
  submit() {
    ...
    if (
      ...
      (path === 'EXISTING_PATIENT_LINK' &&
        !this.form.controls.externalPatientReference.value.trim())
    ) {
      this.form.markAllAsTouched();
      this.error.set('Complete the required details and consent before continuing.');
      return;
    }
    this.saving.set(true);
    const call =
      path === 'NEW_PATIENT_REGISTRATION'
        ? this.api.startNew({ providerReference: p.providerReference, consentAcknowledged: true })
        : this.api.startExisting({
            providerReference: p.providerReference,
            consentAcknowledged: true,
            externalPatientReference: this.form.controls.externalPatientReference.value.trim(),
          });
    call
      .pipe(finalize(() => this.saving.set(false)))
      .subscribe({
        next: (c) =>
          this.router.navigate(['/me/providers', c.reference], {
            queryParams: this.returnUrl ? { returnUrl: this.returnUrl } : undefined,
          }),
        error: () => this.error.set('We couldnt connect you to this hospital. Please try again.'),
      });
  }
  backUrl() {
    return this.returnUrl || '/me/providers';
  }
