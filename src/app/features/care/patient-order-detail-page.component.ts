...
@if (o.type === 'LABORATORY' || o.type === 'IMAGING') {
  @if (fulfillment(); as f) {
...
export class PatientOrderDetailPageComponent {
 ...
 load(){this.api.getPatientOrder(this.ref).pipe(finalize(()=>this.loading.set(false))).subscribe({next:o=>{this.order.set(o);if(o.fulfillment?.reference)this.api.getPatientFulfillment(o.fulfillment.reference).subscribe({next:f=>this.fulfillment.set(f)});else if(o.type==='LABORATORY'||o.type==='IMAGING')this.search()},error:()=>this.error.set('This clinical request is unavailable.')})}
 search(){const o=this.order();if(!o||!['LABORATORY','IMAGING'].includes(o.type))return;this.searching.set(true);this.api.searchFulfillmentProviders(o.type as 'LABORATORY'|'IMAGING',{q:this.form.controls.q.value,page:1,limit:20}).pipe(finalize(()=>this.searching.set(false))).subscribe({next:p=>this.providers.set(p.items),error:()=>this.error.set('Connected providers could not be loaded.')})}
