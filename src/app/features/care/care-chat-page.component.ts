              ...
              >
                ...
                @if (message.attachments.length) {
                  <div class="mt-2 grid gap-2">
                    @for (attachment of message.attachments; track attachment.reference) {
                      <div
                        class="rounded-xl border p-3"
                        [class.border-brand-500]="isMine(message)"
                        [class.bg-brand-800]="isMine(message)"
                        [class.bg-slate-50]="!isMine(message)"
                      >
                        <div class="flex items-start gap-2">
                          <span aria-hidden="true" class="font-bold">{{
                            attachment.resourceType === 'IMAGE' ? 'IMG' : 'PDF'
                          }}</span>
                          <div class="min-w-0">
                        ...
                        <button
                          type="button"
                          (click)="viewAttachment(message, attachment)"
                          [disabled]="accessPending() === attachment.reference"
                          class="mt-2 text-sm font-bold underline"
                        >
                          {{
                            accessPending() === attachment.reference
                              ? 'Opening…'
        ...
        @if (detail.canSendMessages) {
          ...
          >
              ...
              <div class="mt-3 grid gap-2 sm:grid-cols-2">
                @for (attachment of pendingAttachments(); track attachment.reference) {
                  <div class="rounded-xl border bg-slate-50 p-3">
                    ...
                    <button
                      type="button"
                      (click)="removePending(attachment.reference)"
                      class="mt-1 text-sm font-bold text-red-700 underline"
                    >
                      Remove
                    </button>
                  </div>
                ...
                @if (failedFile(); as file) {
                  <div class="rounded-xl border border-red-200 bg-red-50 p-3">
                    <p class="break-words text-sm font-bold">{{ file.name }}</p>
                    <p class="text-xs text-red-700">Upload failed</p>
            ...
            <div class="mt-2 flex items-end gap-3">
              ...
              ></textarea
              ><label
                class="grid min-h-12 cursor-pointer place-items-center rounded-xl border px-4 font-bold"
                >Attach file<input
                  #fileInput
                  type="file"
    ...
    @if (previewUrl(); as url) {
      <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4">
        <section
          role="dialog"
          aria-modal="true"
          aria-labelledby="chat-attachment-preview-title"
        ...
        >
          ...
          <img [src]="url" [alt]="previewName()" class="mt-4 max-h-[75vh] w-full object-contain" />
        </section>
      </div>
    }
  </main>`,
...
export class CareChatPageComponent {
  ...
  canSend(): boolean {
    return (
      !this.sending() &&
      !this.uploadingFile() &&
      (!!this.composer.controls.body.value.trim() || this.pendingAttachments().length > 0)
    );
  }
  ...
  private validateAndUpload(file: File): void {
    ...
    if (!['application/pdf', 'image/jpeg', 'image/png', 'image/webp'].includes(file.type)) {
      this.error.set('Choose a PDF, JPG, PNG or WEBP file.');
      return;
    }
    if (file.size > 15 * 1024 * 1024) {
      this.error.set('The selected file must not exceed 15 MB.');
      return;
    }
    if (this.pendingAttachments().length >= 5) {
      this.error.set('A message can include at most 5 attachments.');
      return;
    }
    this.uploadingFile.set(file);
    const upload =
      this.scope === 'patient'
        ? this.api.uploadPatientAttachment(this.reference, file)
        : this.api.uploadProviderAttachment(this.reference, file);
    upload.pipe(finalize(() => this.uploadingFile.set(null))).subscribe({
      next: (attachment) => this.pendingAttachments.update((items) => [...items, attachment]),
      error: () => {
        this.failedFile.set(file);
        this.error.set('Unable to upload this file. Please try again.');
      },
    });
  }
  retryUpload(): void {
  ...
  removePending(reference: string): void {
    this.pendingAttachments.update((items) => items.filter((item) => item.reference !== reference));
  }
  viewAttachment(message: CareChatMessage, attachment: CareMessageAttachment): void {
    if (this.accessPending()) return;
    this.accessPending.set(attachment.reference);
    this.error.set(null);
    const access =
      this.scope === 'patient'
        ? this.api.getPatientAttachmentAccess(
            this.reference,
            message.reference,
            attachment.reference,
          )
        : this.api.getProviderAttachmentAccess(
            this.reference,
            message.reference,
            attachment.reference,
          );
    access
      .pipe(finalize(() => this.accessPending.set(null)))
      .subscribe({
        next: (value) => this.openAuthorizedAttachment(attachment, value.url),
        error: () => this.error.set('Unable to open this file. Please try again.'),
      });
  }
  private openAuthorizedAttachment(attachment: CareMessageAttachment, value: string): void {
    let url: URL;
    try {
      url = new URL(value);
    } catch {
    ...
    if (attachment.resourceType === 'IMAGE') {
      this.previewUrl.set(url.toString());
  ...
  formatFileSize(bytes: number): string {
    ...
    if (bytes < 1024 * 1024) return `${this.compact(bytes / 1024)} KB`;
    return `${this.compact(bytes / (1024 * 1024))} MB`;
  }
