...
@Component({
  ...
  template: `<main class="mx-auto max-w-5xl px-5 py-10 sm:px-8">
    ...
    @else { <div class="mt-8 grid gap-5">@for (request of items(); track request.reference) {
      ...
      <dl class="mt-5 grid gap-4 sm:grid-cols-2"><div><dt class="text-sm text-slate-500">Requested access</dt><dd class="font-bold">{{scopeLabel(request)}}</dd><dd class="mt-1 text-sm text-slate-600">{{scopeDescription(request)}}</dd></div><div><dt class="text-sm text-slate-500">Reason</dt><dd>{{request.reason}}</dd></div><div><dt class="text-sm text-slate-500">Requested until</dt><dd>{{request.requestedExpiresAt ? date(request.requestedExpiresAt) : 'No requested expiry'}}</dd></div><div><dt class="text-sm text-slate-500">Request date</dt><dd>{{date(request.createdAt)}}</dd></div></dl>
...
export class HealthRecordAccessRequestsPageComponent {
  ...
  scopeLabel(request: ClinicalRecordAccessRequest) { return request.scope === 'HEALTH_PASSPORT' ? 'Health Passport' : request.scope === 'ALL_RECORDS' ? 'Health Passport + finalized clinical records' : request.scope === 'RECORD_TYPE' ? `${this.label(request.recordType ?? '')} finalized records` : `Single finalized record (${request.clinicalRecordReference})`; }
  scopeDescription(request: ClinicalRecordAccessRequest) { return request.scope === 'HEALTH_PASSPORT' ? 'Allows this provider to view clinically relevant information from your Health Passport while this sharing permission remains active.' : request.scope === 'ALL_RECORDS' ? 'Includes your Shareable Health Passport and eligible finalized clinical records.' : 'Only the finalized clinical records identified by this request are covered.'; }
