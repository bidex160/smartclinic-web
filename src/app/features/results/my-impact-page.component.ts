  ...
  template: `
    <main class="mx-auto max-w-7xl px-5 py-10 sm:px-8 lg:py-14">
      ...
      @if (loading()) {
        <div
          role="status"
          aria-label="Loading your impact"
        ...
        <section
          role="alert"
          class="mt-8 rounded-2xl border border-red-200 bg-red-50 p-6 text-red-900"
        >
          <h2 class="font-bold">We couldn't load your impact right now.</h2>
          <button
            type="button"
            (click)="load()"
            class="mt-4 min-h-11 rounded-lg border border-red-300 px-4 font-bold focus:ring-4 focus:ring-red-200"
          >
            Try again
          </button>
        </section>
        ...
        <section class="mt-8 rounded-2xl border bg-white p-6" aria-labelledby="earning-heading">
          ...
          <div class="mt-5 grid gap-6 md:grid-cols-2">
            <article>
              ...
              <dl class="mt-3 divide-y rounded-xl border">
                @for (row of providerRewards; track row.label) {
                  <div class="flex justify-between gap-4 p-3">
                    <dt>{{ row.label }}</dt>
                    <dd class="font-bold">{{ row.points }}</dd>
                  </div>
                }
              </dl>
            </article>
            <article>
              ...
              <dl class="mt-3 divide-y rounded-xl border">
                @for (row of patientRewards; track row.label) {
                  <div class="flex justify-between gap-4 p-3">
                    <dt>{{ row.label }}</dt>
                    <dd class="font-bold">{{ row.points }}</dd>
                  </div>
        ...
        <section class="mt-8 rounded-2xl border bg-white p-6" aria-labelledby="builder-heading">
          ...
          @if (value.levelProgress.currentLevel; as current) {
            <p class="mt-4 text-xl font-bold">{{ current.name }} achieved</p>
          } @else {
              ...
              @for (requirement of value.levelProgress.requirements; track requirement.targetType) {
                <article class="rounded-xl bg-slate-50 p-4">
                  ...
                  <p class="mt-1 text-sm">
                    {{ requirement.qualified }} qualified of {{ requirement.required }} required
                  </p>
                  <p class="mt-2 font-semibold">
                    {{ requirement.completed ? 'Completed' : requirement.remaining + ' remaining' }}
                  </p>
                  <progress
                    class="mt-3 h-2 w-full"
                    [value]="requirement.qualified"
                    [max]="requirement.required || 1"
                  >
                    {{ requirement.qualified }} of {{ requirement.required }}
                  </progress>
                </article>
              }
            </div>
        ...
        <div class="mt-8 grid gap-6 lg:grid-cols-3">
          <section class="rounded-2xl border bg-white p-6">
            ...
            <button
              type="button"
              (click)="copyCode()"
              class="mt-4 min-h-11 rounded-lg bg-brand-700 px-4 font-bold text-white focus:ring-4 focus:ring-brand-200"
            >
              Copy code
            </button>
            ...
            @for (target of inviteTargets(); track target) {
              <article class="rounded-2xl border bg-white p-5">
                ...
                <div class="mt-4 flex flex-wrap gap-2">
                  <button
                    type="button"
                    (click)="copyInvite(target)"
                    class="min-h-11 rounded-lg bg-brand-700 px-4 font-bold text-white focus:ring-4 focus:ring-brand-200"
                  >
                    Copy invite link
                  </button>
                  @if (canShare) {
                    <button
                      type="button"
                      (click)="shareInvite(target)"
                      class="min-h-11 rounded-lg border border-brand-700 px-4 font-bold text-brand-700 focus:ring-4 focus:ring-brand-200"
                    >
                      Share
                    </button>
                  }
        ...
        <div class="mt-8 grid gap-6 lg:grid-cols-2">
          ...
          <section class="rounded-2xl border bg-white p-6" aria-labelledby="leaderboard-heading">
            ...
            <p class="mt-4 font-semibold">
              Current state: {{ value.leaderboard.optedIn ? 'Participating' : 'Not participating' }}
            </p>
            @if (value.leaderboard.optedIn) {
              <p class="mt-1">
                {{
                  value.leaderboard.position
                    ? 'Your current position: #' + value.leaderboard.position
                    : "You're participating, but you're not ranked yet."
                }}
              </p>
            }
            <button
              type="button"
              (click)="updateLeaderboard(!value.leaderboard.optedIn)"
              [disabled]="preferenceUpdating()"
              class="mt-5 min-h-11 rounded-lg border border-brand-700 px-4 font-bold text-brand-700 focus:ring-4 focus:ring-brand-200 disabled:opacity-50"
            >
              {{
                preferenceUpdating()
                  ? 'Updating preference…'
                  : value.leaderboard.optedIn
            ...
            <p
              ...
              [class.text-red-800]="preferenceError()"
            >
              {{ preferenceFeedback() }}
            </p>
          </section>
...
export class MyImpactPageComponent {
  ...
  readonly impact = signal<ReferralImpact | null>(null);
  readonly loading = signal(false);
  readonly error = signal(false);
  readonly preferenceUpdating = signal(false);
  readonly preferenceError = signal(false);
  readonly preferenceFeedback = signal('');
  ...
  readonly providerRewards = [
    ...
    { label: 'Activated', points: '+8' },
    { label: 'Maximum', points: '14' },
  ];
  readonly patientRewards = [
    { label: 'Registered', points: '+1' },
    { label: 'First completed care', points: '+1' },
    { label: 'Maximum', points: '2' },
  ];
  readonly inviteTargets = computed(() => {
    const links = this.impact()?.inviteLinks;
    return links ? this.targets.filter((target) => Boolean(links[target])) : [];
  });
  constructor() {
    this.load();
  }
  load(): void {
    ...
    this.api
      ...
      .subscribe({ next: (value) => this.impact.set(value), error: () => this.error.set(true) });
  }
  pointMetrics(value: ReferralImpact) {
    return [
      { label: 'Available points', value: value.balances.availablePoints },
      { label: 'Lifetime earned points', value: value.balances.lifetimeEarnedPoints },
      { label: 'Reserved points', value: value.balances.reservedPoints },
      { label: 'Lifetime redeemed points', value: value.balances.lifetimeRedeemedPoints },
    ];
  }
  targetLabel(target: ReferralTargetType) {
    return (
      {
        PATIENT: 'Patients',
        CLINIC: 'Clinics',
  ...
  inviteLabel(target: ReferralTargetType) {
    return (
      {
        PATIENT: 'Patient',
        CLINIC: 'Clinic',
  ...
  inviteHelp(target: ReferralTargetType) {
    return target === 'PATIENT'
      ? 'Invite someone to create a SmartClinic patient account.'
      : `Invite a ${this.inviteLabel(target).toLowerCase()} to join SmartClinic.`;
  }
  async copyCode(): Promise<void> {
    const code = this.impact()?.referralCode;
    if (!code) return;
    try {
      await navigator.clipboard.writeText(code);
      this.copyFeedback.set('Referral code copied.');
    } catch {
      this.copyFeedback.set('Copy was unavailable. Select the code and copy it manually.');
    }
  }
  async copyInvite(target: ReferralTargetType): Promise<void> {
    const link = this.impact()?.inviteLinks[target];
    if (!link) return;
    try {
      await navigator.clipboard.writeText(link);
      this.inviteFeedback.set(`${this.inviteLabel(target)} invite link copied.`);
    } catch {
      this.inviteFeedback.set('Copy was unavailable. Try again.');
    }
  }
  async shareInvite(target: ReferralTargetType): Promise<void> {
    const link = this.impact()?.inviteLinks[target];
    if (link && navigator.share)
      await navigator
        .share({
          title: `SmartClinic ${this.inviteLabel(target)} invitation`,
          text: 'Join SmartClinic using my referral link.',
          url: link,
        })
        .catch(() => undefined);
  }
  updateLeaderboard(publicLeaderboard: boolean): void {
    if (this.preferenceUpdating()) return;
    this.preferenceUpdating.set(true);
    this.preferenceError.set(false);
    this.preferenceFeedback.set('');
    this.api.updateLeaderboardPreference(publicLeaderboard).subscribe({
      next: () => {
        this.preferenceUpdating.set(false);
        this.preferenceFeedback.set(
          publicLeaderboard
            ? 'You are now participating in the public leaderboard.'
            : 'You have left the public leaderboard.',
        );
        this.reloadImpact();
      },
      error: () => {
        ...
        this.preferenceFeedback.set('We could not update your leaderboard preference. Try again.');
      },
    });
  }
  private reloadImpact(): void {
    this.api.getMyImpact().subscribe({
      next: (value) => this.impact.set(value),
      error: () => {
        this.preferenceError.set(true);
        this.preferenceFeedback.set(
          'Preference saved, but we could not refresh your position yet.',
        );
      },
    });
  }
