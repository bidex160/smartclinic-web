...
describe('ReferralsPageComponent', () => {
  ...
  it('preserves balances, withdrawal, and target-aware invite links', async () => {
    const writeText=vi.fn().mockResolvedValue(undefined);Object.defineProperty(navigator,'clipboard',{configurable:true,value:{writeText}});const value=summary();const fixture=await setup(value);fixture.detectChanges();const text=fixture.nativeElement.textContent as string;expect(text).toContain('Reserved');expect(text).toContain('Withdrawals');expect(text).toContain('Health Checks');expect(text).toContain('Request cash withdrawal');expect(text).toContain('Invite a Laboratory');await fixture.componentInstance.copyLink('LABORATORY');expect(writeText).toHaveBeenCalledWith(new URL(value.links.LABORATORY,window.location.origin).toString());
  });
});

function summary(levelProgress: ReferralSummary['levelProgress'] = { currentLevel:null,nextLevel:{code:'LEVEL_1',name:'Level 1',ordinal:1},highestLevelAchieved:0,requirements:[{targetType:'PATIENT',qualified:3,required:10,remaining:7,completed:false}],highestConfiguredLevelReached:false,qualifiedCounts:{PATIENT:3,INDIVIDUAL:0,CLINIC:1,LABORATORY:0,PHARMACY:0} }): ReferralSummary { return { referralCode:'SC-ABC123',links:{PATIENT:'/register?ref=SC-ABC123',INDIVIDUAL:'/provider/register?ref=SC-ABC123&type=INDIVIDUAL',CLINIC:'/provider/register?ref=SC-ABC123&type=CLINIC',LABORATORY:'/provider/register?ref=SC-ABC123&type=LABORATORY',PHARMACY:'/provider/register?ref=SC-ABC123&type=PHARMACY'},availablePoints:340,reservedPoints:60,withdrawalReservedPoints:40,healthCheckReservedPoints:20,lifetimeEarnedPoints:500,lifetimeRedeemedPoints:100,levelProgress,currentLevel:null,nextLevel:null,progress:{patients:{qualified:0,required:0},individuals:{qualified:0,required:0},clinics:{qualified:0,required:0},laboratories:{qualified:0,required:0},pharmacies:{qualified:0,required:0}},completed:false,registeredDirectReferrals:10,qualifiedDirectReferrals:7 }; }
