...
@Component({
  ...
  template: `<main class="mx-auto max-w-6xl px-5 py-10 sm:px-8">
    ...
    @if (loading()) {
      <p role="status" class="mt-7 rounded-xl bg-brand-50 p-5">
        Loading your referral and reward information…
      </p>
    }
    @if (error()) {
      <div role="alert" class="mt-7 rounded-xl bg-red-50 p-5 text-red-900">
        <p>{{ error() }}</p>
        <button type="button" (click)="load()" class="mt-3 rounded-lg border px-4 py-2 font-bold">
          Try again
        </button>
      </div>
    }
    @if (summary(); as s) {
      ...
      >
        ...
        <article class="rounded-2xl border bg-white p-5">
          ...
          <dl class="mt-3 grid gap-1 text-sm text-slate-600">
            ...
            <div class="flex justify-between gap-3">
              <dt>Health Checks</dt>
              <dd>{{ s.healthCheckReservedPoints }}</dd>
            </div>
          </dl>
        </article>
        <article class="rounded-2xl border bg-white p-5">
      ...
      <section class="mt-8 rounded-2xl border bg-white p-6" aria-labelledby="withdraw-heading">
        <div class="flex flex-wrap items-start justify-between gap-4">
          ...
          <button
            type="button"
            (click)="openRequest()"
            [disabled]="s.availablePoints < 1 || withdrawalUnavailable()"
        ...
        @if (withdrawalUnavailable()) {
          <p role="alert" class="mt-4 rounded-lg bg-amber-50 p-3 text-amber-950">
            Cash withdrawal is temporarily unavailable because no active reward conversion rate is
            configured.
          </p>
        }
        @if (actionMessage()) {
        ...
        @if (withdrawals().length) {
          <div class="mt-4 overflow-x-auto rounded-xl border">
            <table class="min-w-[820px] w-full text-left">
              ...
              <tbody>
                @for (item of withdrawals(); track item.withdrawalReference) {
                  <tr class="border-t">
                    <td class="p-4 font-semibold">{{ item.withdrawalReference }}</td>
                    ...
                    <td class="p-4">{{ utils.formatDateTime(item.requestedAt) }}</td>
                    <td class="p-4">
      ...
      <div class="mt-8 grid gap-5 lg:grid-cols-3">
        <section class="rounded-2xl border bg-white p-6">
          ...
          <button
            type="button"
            (click)="copyCode()"
            class="mt-4 rounded-lg border border-brand-700 px-4 py-2 font-bold text-brand-700"
          >
            Copy code
          </button>
      ...
      >
        ...
        <div class="mt-4 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          @for (target of targetTypes; track target) {
            <article>
              <h3 class="font-semibold">{{ targetLabelPlural(target) }}</h3>
              <p class="mt-1 text-2xl font-bold">{{ s.levelProgress.qualifiedCounts[target] }}</p>
            </article>
          }
        </div>
      </section>
            ...
            @for (requirement of s.levelProgress.requirements; track requirement.targetType) {
              <article class="rounded-xl bg-slate-50 p-4">
                ...
                @if (requirement.completed) {
                  <p class="mt-2 font-semibold text-brand-800">✓ Completed</p>
                } @else {
                  <p class="mt-2">{{ requirement.remaining }} remaining</p>
                }
              </article>
            }
          </div>
          ...
          @for (invite of invites; track invite.target) {
            <article class="rounded-2xl border bg-white p-5">
              ...
              <button
                ...
                class="mt-4 rounded-lg bg-brand-700 px-4 py-2 font-bold text-white"
              >
                Copy invite link
              </button>
              @if (canShare) {
                <button
                  type="button"
                  (click)="share(invite.target, invite.label)"
      ...
      <section class="mt-8">
        ...
        @if (history().length) {
            ...
            <table class="min-w-[700px] w-full text-left">
              <thead class="bg-slate-50">
                <tr>
                  ...
                  <th class="p-4">Points earned</th>
                </tr>
              </thead>
              <tbody>
                @for (item of history(); track $index) {
                  <tr class="border-t">
                    <td class="p-4">{{ targetLabel(item.targetType) }}</td>
                    ...
                    <td class="p-4">{{ item.pointsEarned ?? 0 }}</td>
                  </tr>
                }
              </tbody>
            </table>
          ...
          <p class="mt-4 rounded-xl border bg-white p-6">
            No referrals yet. Share an invite link to get started.
          </p>
        }
      </section>
    }
    @if (requestOpen() && summary(); as s) {
      <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
        ...
        >
          ...
          <form [formGroup]="requestForm" (ngSubmit)="submitRequest()" class="mt-5 grid gap-4">
            ...
            ><label class="font-semibold"
              >Bank name<input
                ...
                class="mt-1 w-full rounded-lg border p-3"
                placeholder="e.g. Access Bank" /></label
            ><label class="font-semibold"
              >Account number<input
                formControlName="accountNumber"
                inputmode="numeric"
                class="mt-1 w-full rounded-lg border p-3"
                placeholder="e.g. 0123456789" /></label
            ><label class="font-semibold"
              >Account name<input
            ...
            @if (requestError()) {
              <p role="alert" class="rounded-lg bg-red-50 p-3 text-red-900">{{ requestError() }}</p>
            }
            <div class="flex justify-end gap-3">
              ...
              ><button
                type="submit"
                [disabled]="requestForm.invalid || mutating()"
                class="rounded-lg bg-brand-700 px-4 py-2 font-bold text-white disabled:opacity-50"
              >
                {{ mutating() ? 'Submitting withdrawal…' : 'Submit withdrawal' }}
              </button>
            </div>
          </form>
        </section>
        ...
        >
          ...
          <div class="mt-6 flex justify-end gap-3">
            ...
            ><button
              ...
              (click)="confirmCancel()"
              [disabled]="mutating()"
              class="rounded-lg bg-red-700 px-4 py-2 font-bold text-white disabled:opacity-50"
            >
              {{ mutating() ? 'Cancelling…' : 'Cancel withdrawal' }}
            </button>
          </div>
        </section>
      </div>
...
export class ReferralsPageComponent {
  ...
  private readonly withdrawalsApi = inject(RewardWithdrawalsApiService);
  ...
  readonly cancelTarget = signal<RewardWithdrawal | null>(null);
  readonly mutating = signal(false);
  readonly requestError = signal<string | null>(null);
  readonly actionMessage = signal('');
  readonly withdrawalUnavailable = signal(false);
  ...
  readonly invites = [
    {
      ...
      label: 'Patient',
      copy: 'Earn points after their first qualifying Health Check.',
    },
    {
      target: 'CLINIC' as const,
      label: 'Clinic',
      copy: 'Earn points after the provider is approved and active.',
    },
    {
  ...
  readonly targetTypes: ReferralTargetType[] = [
    'PATIENT',
    'INDIVIDUAL',
    'CLINIC',
    'LABORATORY',
  ...
  openRequest() {
    const max = this.summary()?.availablePoints ?? 0;
    if (max < 1 || this.withdrawalUnavailable()) return;
    this.requestForm.reset({
      points: Math.min(max, 1),
      bankName: '',
      accountNumber: '',
      accountName: '',
    });
  ...
  closeRequest() {
    if (!this.mutating()) this.requestOpen.set(false);
  }
  submitRequest() {
    const max = this.summary()?.availablePoints ?? 0;
    if (this.requestForm.invalid || this.mutating()) return;
    const value = this.requestForm.getRawValue();
    if (value.points > max) {
      this.requestError.set('Enter no more than your available points.');
      return;
    }
      ...
      .subscribe({
        next: () => {
          ...
          this.actionMessage.set(
            'Withdrawal request submitted. Your requested points are now reserved.',
          );
          this.load();
        },
        error: (e: HttpErrorResponse) => {
          if (e.status === 409 && /conversion/i.test(String(e.error?.message ?? ''))) {
            this.withdrawalUnavailable.set(true);
            this.requestOpen.set(false);
          } else
            this.requestError.set(
              e.status === 409
                ? 'The withdrawal could not be submitted. Check your available points and try again.'
                : 'We could not submit your withdrawal request.',
            );
        },
      });
  }
  openCancel(item: RewardWithdrawal) {
  ...
  confirmCancel() {
    ...
    this.mutating.set(true);
    this.withdrawalsApi
      .cancelMine(item.withdrawalReference)
      .pipe(finalize(() => this.mutating.set(false)))
      .subscribe({
        next: () => {
          this.cancelTarget.set(null);
          this.actionMessage.set(
            'Withdrawal request cancelled. Reserved points are available again.',
          );
          this.load();
        },
        error: () => {
          this.cancelTarget.set(null);
          this.actionMessage.set(
            'We could not cancel this withdrawal request. Refresh and try again.',
          );
        },
      });
  }
  fullLink(t: ReferralTargetType) {
    const p = this.summary()?.links[t];
    return p ? new URL(p, window.location.origin).toString() : '';
  }
  async copyCode() {
    const c = this.summary()?.referralCode;
    if (c)
      try {
        await navigator.clipboard.writeText(c);
        this.copyFeedback.set('Referral code copied.');
      } catch {
        this.copyFeedback.set('Copy was unavailable.');
      }
  }
  async copyLink(t: ReferralTargetType) {
    const u = this.fullLink(t);
    if (u)
      try {
        await navigator.clipboard.writeText(u);
        this.inviteFeedback.set(`${this.targetLabel(t)} invite link copied.`);
      } catch {
        this.inviteFeedback.set('Copy was unavailable.');
      }
  }
  async share(t: ReferralTargetType, l: string) {
    const u = this.fullLink(t);
    if (u && navigator.share)
      await navigator
        .share({ title: `SmartClinic ${l} invitation`, url: u })
        .catch(() => undefined);
  }
  targetLabel(t: ReferralTargetType) {
    return (
      {
        PATIENT: 'Patient',
        CLINIC: 'Clinic',
        ...
        PHARMACY: 'Pharmacy',
        INDIVIDUAL: 'Individual health professional',
      } as const
    )[t];
  }
  targetLabelPlural(t: ReferralTargetType) {
    return (
      {
        PATIENT: 'Patients',
        CLINIC: 'Clinics',
