    ...
    <main class="mx-auto max-w-3xl px-5 py-12 sm:px-8">
        ...
        <section class="mt-6 rounded-2xl border bg-white p-7">
          @if (payment.fundingStatus === 'SETTLED') {
            ...
            <button
              type="button"
              (click)="verify()"
              class="mt-4 rounded-xl border border-brand-700 px-5 py-3 font-bold text-brand-700"
            >
              Try verification again
            </button>
          }
          <div class="mt-6 flex flex-wrap gap-3">
            <a
              [routerLink]="['/me/health-checks', reference]"
              class="rounded-xl bg-brand-700 px-5 py-3 font-bold text-white"
              >View Health Check</a
            ><a routerLink="/me/health-checks" class="rounded-xl border px-5 py-3 font-bold"
              >Back to My Health Checks</a
            >
          </div>
        </section>
      }
      @if (fastTrackStatus(); as payment) {
        <section class="mt-6 rounded-2xl border bg-white p-7">
          ...
          <div class="mt-6 flex flex-wrap gap-3">
            <a
              [routerLink]="['/me/fasttrack', reference]"
              class="rounded-xl bg-brand-700 px-5 py-3 font-bold text-white"
              >View FastTrack request</a
            ><a routerLink="/me/fasttrack" class="rounded-xl border px-5 py-3 font-bold"
              >Back to FastTrack requests</a
            >
          </div>
        </section>
      }
      @if (careFunding(); as payment) {
        <section class="mt-6 rounded-2xl border bg-white p-7">
          ...
          <div class="mt-6 flex flex-wrap gap-3">
            <a
              [routerLink]="['/me/care', reference]"
              class="rounded-xl bg-brand-700 px-5 py-3 font-bold text-white"
              >View Care Request</a
            ><a routerLink="/me/care" class="rounded-xl border px-5 py-3 font-bold"
              >Back to My Care</a
            >
          </div>
        </section>
      ...
      @if (connectionFunding(); as payment) {
        <section class="mt-6 rounded-2xl border bg-white p-7">
          ...
          <a
            [routerLink]="['/me/providers', reference]"
            class="mt-6 inline-flex rounded-xl bg-brand-700 px-5 py-3 font-bold text-white"
            >View Provider connection</a
          >
        </section>
      }
  ...
  verify(): void {
    ...
    if (this.reference.startsWith('SC-FT-')) {
      ...
      fastTrack
        ...
        .subscribe({
          next: (payment) => this.fastTrackStatus.set(payment),
    ...
    if (this.reference.startsWith('SC-PPC-')) {
      const connections = this.injector.get(PatientProviderConnectionsApiService);
      connections
        .verifyFunding(this.reference)
        .pipe(finalize(() => this.verifying.set(false)))
        .subscribe({
          next: (f) => this.connectionFunding.set(f),
          error: () => this.error.set(true),
        });
      return;
    }
    if (this.reference.startsWith('SC-PHQ-')) {
      const pharmacy = this.injector.get(PharmacyFulfillmentApiService);
      pharmacy
        .verifyFunding(this.reference)
        .pipe(finalize(() => this.verifying.set(false)))
        .subscribe({ next: (f) => this.pharmacyFunding.set(f), error: () => this.error.set(true) });
      return;
    }
    this.api
      .verifyMyHealthCheckPayment(this.reference)
      .pipe(
        switchMap(() =>
          forkJoin({
            status: this.api.getMyHealthCheckPayment(this.reference),
            detail: this.api.getMyHealthCheck(this.reference),
          }),
        ),
