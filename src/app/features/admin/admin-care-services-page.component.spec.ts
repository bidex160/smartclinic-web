...
describe('AdminCareServicesPageComponent', () => {
  ...
  it('creates with the exact DTO and reloads the catalogue', async () => {
    ...
    c.openCreate(); c.form.setValue({ code: 'general_consultation', name: 'General Consultation', description: 'General consultation with a healthcare provider' }); c.submit();
    expect(api.create).toHaveBeenCalledWith({ code: 'GENERAL_CONSULTATION', name: 'General Consultation', description: 'General consultation with a healthcare provider' });
    expect(api.list).toHaveBeenCalledTimes(2);
  });

  it('enforces required backend fields', async () => {
    const { fixture, api } = await setup(); const c = fixture.componentInstance;
    c.openCreate(); c.form.setValue({ code: '', name: '   ', description: '' }); c.submit();
  ...
  it('edits only supported definition fields and reloads', async () => {
    ...
    expect(api.update).toHaveBeenCalledWith('definition-id', { code: 'GENERAL_CONSULTATION', name: 'Primary Care Consultation', description: 'General consultation' });
    expect(api.list).toHaveBeenCalledTimes(2);
  });
  ...
  it('keeps the form open with safe feedback after a backend conflict', async () => {
    ...
    const c = fixture.componentInstance; c.openCreate(); c.form.setValue({ code: 'GENERAL_CONSULTATION', name: 'General Consultation', description: '' }); c.submit();
  ...
  it('prevents duplicate form submission while a mutation is pending', async () => {
    ...
    const c = fixture.componentInstance; c.openCreate(); c.form.setValue({ code: 'GENERAL_CONSULTATION', name: 'General Consultation', description: '' }); c.submit(); c.submit();
