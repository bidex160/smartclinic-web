...
@Component({
  ...
  template: `
    <main class="mx-auto max-w-7xl px-5 py-10 sm:px-8">
      <div class="flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <p class="mt-2 max-w-3xl text-slate-600">
            Manage the General Care services that providers can offer through Find Care.
          </p>
        </div>
        <button
          type="button"
          (click)="openCreate()"
          class="rounded-xl bg-brand-700 px-5 py-3 font-bold text-white focus:outline-none focus:ring-4 focus:ring-brand-200"
        >
          Add Care Service
        </button>
      </div>
      ...
      @if (feedback()) {
        <p aria-live="polite" class="mt-6 rounded-xl bg-green-50 p-4 font-semibold text-green-950">
          {{ feedback() }}
        </p>
      }
      @if (actionError()) {
        <p role="alert" class="mt-6 rounded-xl bg-red-50 p-4 text-red-950">{{ actionError() }}</p>
      }
      @if (loading()) {
        <p role="status" class="mt-8 rounded-2xl border bg-white p-6">Loading Care Services…</p>
      } @else if (loadError()) {
        <div role="alert" class="mt-8 rounded-2xl bg-red-50 p-6 text-red-950">
          We could not load Care Services.
        ...
        <section class="mt-8 rounded-2xl border bg-white p-8 text-center">
          ...
          <button
            type="button"
            (click)="openCreate()"
            class="mt-5 rounded-xl bg-brand-700 px-5 py-3 font-bold text-white"
          >
            Add Care Service
          </button>
        </section>
          ...
          <table class="w-full min-w-[720px] text-left">
            <thead class="bg-slate-50">
              <tr>
                ...
                <th class="p-4">Clinical record</th>
                ...
                <tr class="border-t align-top">
                  ...
                  <td class="p-4">
                    <span
                      ...
                      [class.bg-green-100]="definition.isActive"
                      [class.text-green-900]="definition.isActive"
                      [class.bg-slate-200]="!definition.isActive"
                      >{{ definition.isActive ? 'Active' : 'Inactive' }}</span
                    >
                  </td>
                  <td class="p-4">
                    <div class="flex flex-wrap gap-3">
                      ...
                      ><button
                        type="button"
                        (click)="setActive(definition, !definition.isActive)"
                        [disabled]="mutatingId() === definition.id"
                        class="font-bold text-brand-700 underline disabled:opacity-50"
                      >
      ...
      @if (modalOpen()) {
        <div
          ...
          role="dialog"
          aria-modal="true"
          aria-labelledby="care-service-form-title"
        >
          <section
            class="max-h-[90vh] w-full max-w-xl overflow-y-auto rounded-2xl bg-white p-6 shadow-xl sm:p-8"
          >
            <h2 id="care-service-form-title" class="text-2xl font-bold">
              {{ editing() ? 'Edit Care Service' : 'Add Care Service' }}
            </h2>
            <p class="mt-2 text-slate-600">
              Define what General Care service exists across SmartClinic. Providers configure
              delivery and pricing separately.
            </p>
            <form [formGroup]="form" (ngSubmit)="submit()" class="mt-6 grid gap-5" novalidate>
              <label class="font-semibold"
                >Code<input
                  ...
                  class="mt-2 block w-full rounded-xl border p-3 font-mono uppercase"
                  aria-describedby="code-help code-error"
                /><span id="code-help" class="mt-1 block text-sm font-normal text-slate-500"
                  >Uppercase letters, numbers, and underscores. This identifier is required by the
                  backend.</span
                >
                @if (form.controls.code.touched && form.controls.code.invalid) {
                  <span id="code-error" class="mt-1 block text-sm font-normal text-red-700"
                    >Enter 2–80 characters beginning with a letter.</span
                  >
                }
              </label>
              <label class="font-semibold"
                ...
                @if (form.controls.name.touched && form.controls.name.invalid) {
                  <span class="mt-1 block text-sm font-normal text-red-700"
                    >Name is required and must be no more than 160 characters.</span
                  >
                }
              </label>
              <label class="font-semibold"
                ...
                ><textarea
                  ...
                  maxlength="4000"
                  rows="5"
                  placeholder="General consultation with a healthcare provider"
                  class="mt-2 block w-full rounded-xl border p-3"
                ></textarea>
              </label>
              <label class="font-semibold"
                >Clinical record type<select
                  formControlName="clinicalRecordType"
                  class="mt-2 block min-h-12 w-full rounded-xl border p-3"
                >
                  <option value="">No required clinical record</option>
                  @for (option of clinicalRecordTypes; track option.value) {
                    <option [value]="option.value">{{ option.label }}</option>
                  }</select
                ><span class="mt-1 block text-sm font-normal text-slate-500"
                  >Appointments for this service can only be completed after the configured record
                  is finalized.</span
                ></label
              >
              @if (mutationError()) {
                <p role="alert" class="rounded-xl bg-red-50 p-4 text-red-900">
                  {{ mutationError() }}
                </p>
              }
              <div class="flex flex-col-reverse gap-3 sm:flex-row sm:justify-end">
                ...
                ><button
                  type="submit"
                  [disabled]="saving()"
                  class="rounded-xl bg-brand-700 px-5 py-3 font-bold text-white disabled:opacity-60"
                >
                  {{ saving() ? 'Saving…' : editing() ? 'Save changes' : 'Create Care Service' }}
                </button>
              </div>
            </form>
          </section>
...
export class AdminCareServicesPageComponent {
  ...
  openCreate(): void {
    ...
    this.form.reset({ code: '', name: '', description: '', clinicalRecordType: '' });
  ...
  openEdit(definition: AdminCareServiceDefinition): void {
    ...
    this.form.reset({
      ...
      clinicalRecordType: definition.clinicalRecordType ?? '',
    });
    this.mutationError.set(null);
    this.feedback.set(null);
    this.actionError.set(null);
  ...
  submit(): void {
    ...
    const operation = this.editing()
      ? this.api.update(this.editing()!.id, request)
      : this.api.create(request);
    operation.pipe(finalize(() => this.saving.set(false))).subscribe({
      next: () => {
        this.modalOpen.set(false);
        this.feedback.set(this.editing() ? 'Care Service updated.' : 'Care Service created.');
        this.load();
      },
  ...
  setActive(definition: AdminCareServiceDefinition, isActive: boolean): void {
    ...
    this.mutatingId.set(definition.id);
    this.feedback.set(null);
    this.actionError.set(null);
    this.api
      .update(definition.id, { isActive })
      .pipe(finalize(() => this.mutatingId.set(null)))
      .subscribe({
        next: () => {
          this.feedback.set(isActive ? 'Care Service activated.' : 'Care Service deactivated.');
          this.load();
        },
  ...
  private safeError(error: HttpErrorResponse): string {
    ...
    if ((error.status === 400 || error.status === 409) && typeof message === 'string')
      return message;
